...
index = MasterTable.index.tolist()
#
for i in index:
    person = MasterTable.ix[i]
    MasterTable_noperson = MasterTable[MasterTable.index != i]
    
    unique = CVlib.check_unique(MasterTable_noperson, person)
    
    if (unique != -1):
        print "COPY " + str(unique)
